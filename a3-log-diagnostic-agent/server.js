...
async function run(job) {
  try {
    ...
    job.result = { sn: job.sn, model: modelFromSn(job.sn), source: 'enterprise-agent', ...result };
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === 'GET' && url.pathname === '/api/status') {
    return json(res, 200, { ok: true, mode: process.env.A3_COLLECTOR_COMMAND ? 'enterprise' : 'local', version: '1.0.0' });
  }
...
server.listen(PORT, '0.0.0.0', () => console.log(`A3 Log Diagnostic Agent: http://localhost:${PORT}`));
